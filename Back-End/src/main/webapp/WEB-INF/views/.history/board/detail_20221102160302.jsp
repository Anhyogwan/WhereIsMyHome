								...
								<div class="row">
									...
									<div class="col-lg-6">
										<div class="p-5">
											...
											<form class="mt-5" id="modityForm" method="post"
												action="${root}/board/modifyForm">
												...
												<button type="button" class="btn btn-primary btn-user btn-block"
													id="modifyForm">수정</button>
											</form>
											<a class="mt-3 btn btn-facebook btn-user btn-block"
												href="${root}/board/articles">나가기</a>
											<a class="btn btn-google btn-user btn-block" href="#"
												id="deleteArticle">삭제</a>
											<hr>
												...
												<table>
													...
													<c:if test="${!empty dto.fileInfos}">
														<c:forEach var="file" items="${dto.fileInfos}">
															<tr>
																<th>${file.originalFile} <a href="#" class="filedown"
																		sfolder="${file.saveFolder}"
																		sfile="${file.saveFile}"
																		ofile="${file.originalFile}">[다운로드]</a></th>
															</tr>
														</c:forEach>
													</c:if>
			...
			<script>
              ...
              if(data.fileInfos!=null){
                ...
                document.getElementById("filedoc").innerHTML= `														<tr>
																<th>\${.originalFile} <a href="#" class="filedown"
																		sfolder="${file.saveFolder}"
																		sfile="${file.saveFile}"
																		ofile="${file.originalFile}">[다운로드]</a></th>
															</tr>`;
