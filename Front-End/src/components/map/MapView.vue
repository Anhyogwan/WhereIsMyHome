...
<script
  ...
  src="//dapi.kakao.com/v2/maps/sdk.js?appkey=API키 입력"
></script>
<script>
...
export default {
  ...
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      script.src = "//dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=API키 입력";
  ...
  methods: {
    ...
    async getData(aptCode, startDate = "", endDate = "") {
      await axios
        .get(`http://192.168.120.62/apt/aptchart?aptCode=${aptCode}&startDate=${startDate}&endDate=${endDate}`)
        .then((data) => {
          this.aptlabels = [];
          this.aptdata = [];
