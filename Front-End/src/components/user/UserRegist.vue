  ...
  <div class="text-center" style="margin: 0 auto; width: 600px">
    ...
    <div class="NoticeDiv" style="height: auto">
      <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <v-text-field v-model="code" :rules="authRules" :error-messages="sign" label="인증번호" required></v-text-field>
...
export default {
  ...
  data() {
    return {
      ...
      passwordRules: [
        (v) => !!v || "필수입력 항목입니다.",
        (v) => (v && v.length >= 8 && v.length <= 20) || "비밀번호는 8자 이상 20자 이하만 입력 가능합니다",
  ...
  methods: {
    ...
    checkcode(v) {
      axios
        .post(`http://192.168.120.62/user/checkmail`, { code: v })
        .then(() => {
          this.sign = [];
        })
        .catch(() => {
