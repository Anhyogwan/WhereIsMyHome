<template>
  <v-sheet class="mx-auto" elevation="9" max-width="100%">
    <v-slide-group class="pa-4" actice-class="success" show-arrows>
...
export default {
  ...
  methods: {
    ...
    getRecentInfo() {
      console.log(123);
      this.getRecentlyAptList(this.qty);
      ...
      for (let i = 0; i < this.recentlyAptList.length; i++) {
        ...
        async this.getAptInfoByAptCode(this.recentlyAptList[i].aptCode);
        let url =
